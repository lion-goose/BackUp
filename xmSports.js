/*
 * 鏇存柊鏃堕棿: 2020-11-01 13:43:28
 * TG棰戦亾: https://t.me/ioshkj7
 * 寰俊鍏紬鍙�: ios榛戠鎶€
 * 鍘熶綔鑰呬笉鐭ラ亾鏄皝浜嗭紝鏈汉鍙槸淇锛屾劅璋㈠師浣滆€�
 */
/*
灏忕背杩愬姩淇敼寰俊鏀粯瀹濊繍鍔ㄦ鏁�
APP Store涓嬭浇灏忕背杩愬姩APP
鐧诲叆灏忕背杩愬姩(鐧诲綍鏂瑰紡蹇呴』鏄墜鏈哄彿鐮�+瀵嗙爜(娌℃湁灏辩敤鎵嬫満鍙风爜娉ㄥ唽),涓嬮潰鐨勭涓夋柟璐﹀彿(灏忕背璐﹀彿,Apple,寰俊)鎺堟潈鐧诲綍涓嶈)
鐧诲綍鎴愬姛鍚庡湪 鎴戠殑->绗笁鏂规帴鍏�->缁戝畾鏀粯瀹�,寰俊
灏忕背杩愬姩鍙涓嶉€€鍑虹櫥褰曪紝灏变細鑷姩鑾峰彇鏂扮殑token,鍗虫案涔呮湁鏁�
[MITM]
hostname = account.huami.com
Surge
[Script]
灏忕背杩愬姩 = type=cron,cronexp="15 17 * * *",wake-system=1,timeout=3600,script-path=xmSports.js
灏忕背杩愬姩鑾峰彇Token = type=http-response,pattern=^https:\/\/account\.huami\.com\/v2\/client\/login, requires-body=1, max-size=0, script-path=backUp/xmSports.js
鍦圶
[task_local]
# 灏忕背杩愬姩
15 17 * * * xmSports.js, tag=灏忕背杩愬姩, img-url=https://6678888.xyz/quantumultX/xmbs.png, enabled=true
[rewrite_local]
# 灏忕背杩愬姩鑾峰彇Token
^https:\/\/account\.huami\.com\/v2\/client\/login url script-response-body xmSports.js
Loon
[Script]
cron "15 17 * * *" script-path=xmSports.js, tag=灏忕背杩愬姩
http-response ^https:\/\/account\.huami\.com\/v2\/client\/login script-path=xmSports.js, requires-body=true, timeout=3600, tag=灏忕背杩愬姩鑾峰彇Token
 */

const $ = new Env('灏忕背杩愬姩');
...
let login_token = '';
//闇€瑕佷慨鏀圭殑杩愬姩姝ユ暟娉㈠姩鑼冨洿锛岃剼鏈粯璁や慨鏀规鏁拌寖鍥翠负1w9鍒�2w5
...
function getToken() {
  if ($response.body) {
    ...
    if ($.getdata('xmSportsToken')) {
      $.msg($.name, '鏇存柊Token: 鎴愬姛馃帀', ``);
    } else {
      $.msg($.name, '鑾峰彇Token: 鎴愬姛馃帀', '');
    }
  ...
  if (login_token) {
    ...
    if ($.tokenInfo && $.tokenInfo.result === 'ok') {
      ...
      if ($.changeStepRes && $.changeStepRes.code === 1) {
        console.log(`姝ユ暟淇敼鎴愬姛:${step}姝);
        $.msg($.name, `${step}姝ヰ煆冧慨鏀规垚鍔焋, `鏃堕棿锛�${timeFormat(localtime())}鈥峘, { "open-url": "alipays://platformapi/startapp?appId=20000869" })
      } else {
        console.log(`淇敼杩愬姩姝ユ暟澶辫触`)
      }
    } else {
      $.msg($.name, '澶辫触', `Token宸插け鏁堬紝璇烽噸鏂拌幏鍙朻)
    }
  } else {
    $.log('鏆傛棤Token')
    $.log(`\n\n鑾峰彇TOKEN鏂规硶锛歕nAPP Store涓嬭浇灏忕背杩愬姩APP\n鐧诲叆灏忕背杩愬姩(鐧诲綍鏂瑰紡蹇呴』鏄墜鏈哄彿鐮�+瀵嗙爜(娌℃湁灏辩敤鎵嬫満鍙风爜娉ㄥ唽),涓嬮潰鐨勭涓夋柟璐﹀彿(灏忕背璐﹀彿,Apple,寰俊)鎺堟潈鐧诲綍涓嶈)\n鐧诲綍鎴愬姛鍚庡湪 鎴戠殑->绗笁鏂规帴鍏�->缁戝畾鏀粯瀹�,寰俊\n灏忕背杩愬姩鍙涓嶉€€鍑虹櫥褰曪紝灏变細鑷姩鑾峰彇鏂扮殑token,鍗虫案涔呮湁鏁坄)
    //$.msg($.name, `澶辫触`, '鏆傛棤Token')
  }
  $.done()
}
